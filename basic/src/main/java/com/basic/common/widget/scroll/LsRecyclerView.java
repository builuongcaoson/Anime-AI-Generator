...
public class LsRecyclerView extends RecyclerView implements RecyclerView.OnItemTouchListener {
    ...
    private final LsScroller mScrollbar;
    ...
    public LsRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LsRecyclerView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.LsRecyclerView, 0, 0);

        boolean bouncyRotateEnabled;
        boolean bouncyVerticalEnabled;
        boolean bouncyHorizontalEnabled;

        try {
            mFastScrollEnabled = typedArray.getBoolean(R.styleable.LsRecyclerView_fastScrollThumbEnabled, false);
            bouncyRotateEnabled = typedArray.getBoolean(R.styleable.LsRecyclerView_fastScrollBouncyRotateEnabled, false);
            bouncyVerticalEnabled = typedArray.getBoolean(R.styleable.LsRecyclerView_fastScrollBouncyVerticalEnabled, false);
            bouncyHorizontalEnabled = typedArray.getBoolean(R.styleable.LsRecyclerView_fastScrollBouncyHorizontalEnabled, false);
        } finally {
            typedArray.recycle();
        }

        BaseEdgeEffectFactory baseEdgeEffectFactory = new BaseEdgeEffectFactory();
        baseEdgeEffectFactory.setBouncyRotateEnabled(bouncyRotateEnabled);
        baseEdgeEffectFactory.setBouncyVerticalEnabled(bouncyVerticalEnabled);
        baseEdgeEffectFactory.setBouncyHorizontalEnabled(bouncyHorizontalEnabled);
        setEdgeEffectFactory(baseEdgeEffectFactory);

        mScrollbar = new LsScroller(context, this, attrs);
    ...
    public void attachViewPager(ViewPager2 viewPager2){
        LsRecyclerView.this.viewPager2 = viewPager2;
    }
    ...
    public boolean onInterceptTouchEvent(@NonNull RecyclerView rv, @NonNull MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_MOVE && viewPager2 != null){
    ...
    public String scrollToPositionAtProgress(float touchFraction) {
        ...
        if (getAdapter() instanceof ConcatAdapter) {
            ConcatAdapter adapter = (ConcatAdapter) getAdapter();
            if (adapter.getAdapters().size() == 1) {
                if (adapter.getAdapters().get(0) instanceof SectionedAdapter) {
                    sectionName = ((SectionedAdapter) adapter.getAdapters().get(0)).getSectionName(posInt);
                }
            } else {
                if (posInt == 0) {
                    if (adapter.getAdapters().get(0) instanceof SectionedAdapter) {
                    ...
                    if (adapter.getAdapters().get(1) instanceof SectionedAdapter) {
                        sectionName = ((SectionedAdapter) adapter.getAdapters().get(1)).getSectionName(posInt - 1);
                    }
                }
            }
        }
    ...
    /**
     * Set the FastScroll Popup position. This is either {@link LsScroller.PopupPosition#ADJACENT},
     * meaning the popup moves adjacent to the FastScroll thumb, or {@link LsScroller.PopupPosition#CENTER},
     * meaning the popup is static and centered within the RecyclerView.
     */
    public void setPopupPosition(@LsScroller.PopupPosition int popupPosition) {
