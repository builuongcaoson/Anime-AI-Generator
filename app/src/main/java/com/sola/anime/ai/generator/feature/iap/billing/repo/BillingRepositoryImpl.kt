...
private var isEnableLogging = true

public class BillingRepositoryImpl constructor(
                ...
                if (p0.isOk()) {
                    ...
                    nonConsumableKeys.takeIf { it.isNotEmpty() }
                        ?.queryProductDetails(BillingClient.ProductType.INAPP) { response ->
                            nonConsumableProductDetails.clear()
                            nonConsumableProductDetails.addAll(response.data ?: emptyList())
                    ...
                    consumableKeys.takeIf { it.isNotEmpty() }
                        ?.queryProductDetails(BillingClient.ProductType.INAPP) { response ->
                            consumableProductDetails.clear()
                            consumableProductDetails.addAll(response.data ?: emptyList())
                    ...
                    subscriptionKeys.takeIf { it.isNotEmpty() }
                        ?.queryProductDetails(BillingClient.ProductType.SUBS) { response ->
                            subscriptionProductDetails.clear()
                            subscriptionProductDetails.addAll(response.data ?: emptyList())
    ...
    private fun queryPurchased() {
        Log.e("Main12345","queryPurchased")
            ...
            if (billingResult.isOk()){
                if (purchasedList.isNotEmpty()) {
                    purchasedList.forEach {
                        Log.e("Main12345","queryPurchased 2: ${it.products.joinToString { it }}")
                    ...
                    billingListener?.onPurchasesUpdated(null)
                }
            }
        }
    }
    ...
    private fun handleConsumableProduct(purchase: Purchase) {
        ...
        billingClient.consumeAsync(consumeParams) { billingResult, _ ->
            if (billingResult.isOk()) {
                ...
                billingListener?.cancelPurchase()
            }
        }
    }
    ...
    private fun launchBillingFlow(
        ...
        Log.e("Main12345","Launch billing: $productId --- $type")

        nonConsumableProductDetails.forEach {
            Log.e("Main12345","First: ${it.first == productId}")
        }
        ...
        val params = if (type == BillingClient.ProductType.SUBS){
