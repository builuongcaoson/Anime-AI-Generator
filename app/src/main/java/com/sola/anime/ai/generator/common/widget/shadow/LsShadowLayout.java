package com.sola.anime.ai.generator.common.widget.shadow;
...
public class LsShadowLayout extends FrameLayout {
    ...
    public LsShadowLayout(final Context context) {
        this(context, null);
    }

    public LsShadowLayout(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LsShadowLayout(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.LsShadowLayout);
        try {
            setIsShadowed(typedArray.getBoolean(R.styleable.LsShadowLayout_sl_shadowed, true));
            setShadowRadius(
                    typedArray.getDimension(
                            R.styleable.LsShadowLayout_sl_shadow_radius, DEFAULT_SHADOW_RADIUS
                    )
            );
            setShadowDistance(
                    typedArray.getDimension(
                            R.styleable.LsShadowLayout_sl_shadow_distance, DEFAULT_SHADOW_DISTANCE
                    )
            );
            setShadowAngle(
                    typedArray.getInteger(
                            R.styleable.LsShadowLayout_sl_shadow_angle, (int) DEFAULT_SHADOW_ANGLE
                    )
            );
            setShadowColor(
                    typedArray.getColor(
                            R.styleable.LsShadowLayout_sl_shadow_color, DEFAULT_SHADOW_COLOR
                    )
            );
        } finally {
            typedArray.recycle();
